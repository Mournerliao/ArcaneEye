...
export function HudPanel() {
  const { visible, exiting, mode, title, subtitle, source } = useHudStore(
    useShallow((s) => ({
      visible: s.visible,
      exiting: s.exiting,
      mode: s.mode,
  ...
  /* ── Idle state: show demo launcher ── */
  if (!visible) {
    return (
      <div className="flex h-screen w-screen items-center justify-center gap-3 bg-bg">
        <button
        ...
        <span className="text-xs text-muted">Alt+Q 唤起</span>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center justify-between border-b border-surface-2 px-4 py-3">
        ...
        <button
          onClick={hide}
          className="ml-2 flex h-6 w-6 shrink-0 items-center justify-center rounded-sm text-muted transition-colors hover:text-ink"
          aria-label="关闭"
        >
          ✕
        </button>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-3">
        {mode === "champion" ? <ChampionPanel /> : <HexPanel />}
      </main>
