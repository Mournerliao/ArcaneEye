...
function RankedRow({ item, isTop }: { item: RankedItem; isTop: boolean }) {
  ...
  return (
    <div
      className={`group flex items-center gap-3 rounded-sm px-3 py-2 transition-colors ${
        item.rank === 1
          ? "bg-primary/8 shadow-glow"
          : "hover:bg-surface-2"
      }`}
    >
      {/* Rank */}
      <span className="w-5 shrink-0 text-center font-mono text-sm font-bold text-primary">
        {item.rank}
      </span>
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`truncate leading-tight ${
            item.rank === 1 ? "text-sm font-semibold text-ink" : "text-sm text-ink"
          }`}
        >
          {item.name}
        </p>
      ...
      <span
        className={`shrink-0 font-mono text-base tabular-nums ${
          isTop ? "font-bold text-primary" : "text-muted"
        }`}
      >
