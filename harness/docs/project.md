...
> **项目愿景：** 一个专注于为《英雄联盟》大乱斗玩家提供极致、实时海克斯/出装选择辅助的专用 AI 智能体 (LOL ARAM Dedicated Agent)。
...
### 2. 核心链路流程
`全局快捷键触发 (Alt + Q)` ➔ `截图模块静默捕获屏幕图像` ➔ `上层外壳接收图片并感知上下文` ➔ `触发固定大乱斗 RAG 接口 (如 ARAMGG)` ➔ `大模型多模态处理` ➔ `桌面 HUD 悬浮窗特效滑出反馈`。

### 3. 核心定位与优势
* **专用化定位：** 专注于"海克斯大乱斗助手"，将精力集中在选人与海克斯选择阶段（20-30 秒）的极致打磨。
* **开箱即用：** 内置基本截图能力，无需额外安装任何工具即可使用。
* **HDR 增强（可选）：** 支持集成第三方 HDR 截图工具，解决 Windows 平台 HDR 截图发白问题。

## 二、核心架构设计

项目采用 **模块化** 架构，截图功能作为可插拔模块设计。

### 1. 截图模块设计
* **内置截图能力：**
  - 使用 Tauri Rust 后端调用系统级截图 API
  - 支持 SDR 场景下的全屏和区域截图
  - 开箱即用，无需额外配置

* **HDR 增强（可选）：**
  - 支持通过 HTTP API 与第三方 HDR 截图工具通信
  - 用户可自行安装 HDR 截图工具（如 Lumiere）获得更好的 HDR 体验
  - 自动检测 HDR 工具可用性，无缝切换

### 2. 进程间通讯 (IPC)
* 内置截图模块直接调用系统 API
* 可选的外部工具通过 HTTP API 通信（如 `localhost:18650`）
* 统一的截图接口，支持多种截图后端
